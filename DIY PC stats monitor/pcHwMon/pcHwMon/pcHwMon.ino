...
void setup() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    tft.drawBitmap(x, 0, main_img[i], 64, 64, TFT_RED);
    //main icons for small modules
    tft.drawBitmap(x + 1, 257, main_img[i + 2], 64, 64, TFT_DARKGREY);
    tft.drawBitmap(x, 256, main_img[i + 2], 64, 64, TFT_RED);
    //symbols icons for small modules
    tft.drawBitmap(x + 193, 265, symbols_img[0][0], 48, 48, TFT_WHITE);
    tft.drawBitmap(x + 192, 264, symbols_img[0][0], 48, 48, TFT_RED);
    for (int ii = 1; ii < 4; ii++) {
      int y = 64 * ii;
      //sensors icons
      tft.drawBitmap(x + 36, y + 9, sensors_img[i][ii - 1], 48, 48, TFT_DARKGREY);
      tft.drawBitmap(x + 35, y + 8, sensors_img[i][ii - 1], 48, 48, TFT_RED);
      //symbols icons
      tft.drawBitmap(x + 193, y + 9, symbols_img[i][ii - 1], 48, 48, TFT_WHITE);//WHITE SHADOW
      tft.drawBitmap(x + 192, y + 8, symbols_img[i][ii - 1], 48, 48, TFT_RED);//RED ICON
    }
  }
